  ...
  <div>
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="toggleCheckAll">
                  <span class="checkbox-btn item-check-btn" :class="{'check':checkAllGFlag}">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" @click="checkOut" :class="{'btn--dis':checkedCount===0}">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <Modal :mdShow="modalConfirm" v-on:close="closeModal">
    	...
    	<div class="" slot="btnGroup">
    		<a href="" class="btn btn--m" @click.prevent="delCart()">确认</a>
    		<a href="" class="btn btn--m" @click.prevent="modalConfirm=false">关闭</a>
    	</div>
    </Modal>
    <NavFooter></NavFooter>
  </div>
  ...
  import {currency} from "./../utils/currery"
    export default{
        ...
        methods:{
          ...
          toggleCheckAll(){//全选
            let flag = !this.checkAllGFlag;
            this.cartList.forEach(item=>{
              item.checked = flag?true:false;
            })
            axios.post('/users/editCheckAll',{checkAll:flag}).then(res=>{
              if(res.data.status==0){
                console.log('is toggleCheckAll:'+res.data)
              }
            })
          },
          checkOut(){
