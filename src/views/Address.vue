    ...
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
                ...
                <ul>
                  <li v-for="(item,index) in addressListFilter" :class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                    ...
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" v-if="item.isDefault==0" @click="setDefault(item.addressId)"><i>设为默认地址</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-show="item.isDefault==1">默认地址</div>
                  </li>
                  <li class="addr-new" @click="AddressShow=true">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :class="{'btn--dis':!selectedAddrId}" :to="{path:'orderConfirm',query:{'addressId':selectedAddrId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      ...
      <Modal :mdShow="isMdShow" @close="closeModal">
        ...
        <div class="" slot="btnGroup">
          <a href="" class="btn btn--m" @click.prevent="delAddress">确认</a>
          <a href="" class="btn btn--m" @click.prevent="isMdShow=false">取消</a>
        </div>
      </Modal>
      <!-- 删除弹窗 -->
      <Modal :mdShow="isMdShow2" @close="closeModal">
        <p class="" slot="msg">
          地址列表至少需要有一条数据，无法删除此条数据！
        </p>
        <div class="" slot="btnGroup">
          <a href="" class="btn btn--m" @click.prevent="isMdShow2=false">好的</a>
        </div>
      </Modal>
      <!--添加收货地址-->
      <Modal :mdShow='AddressShow' @close="closeModal">
        <div slot="msg">
          <div class="error-wrap">
            <div slot="mdtitle" style="font-size: 20px;font-weight: 600;margin-bottom:20px">添加收货地址</div>
            <span class="error" v-if="!errmsg"></span>
            <span class="error error-show" v-show="errmsg">{{errmsg}}</span>
          </div>
          <ul>
            <li class="regi_form_input">
              <i class="iconfont icon-xingming"></i>
              <input type="text" tabindex="1" name="loginname" placeholder="请输入收货姓名" class="regi_login_input" v-model="addername">
            </li>
            <li class="regi_form_input noMargin">
              <i class="iconfont icon-phone"></i>
              <input type="text" tabindex="1" name="password" placeholder="请输入收货电话" class="regi_login_input" v-model="phone">
            </li>
            <li class="regi_form_input noMargin">
              <i class="iconfont icon-gongsiyoubian"></i>
              <input type="text" tabindex="1" name="password" placeholder="请输入邮编编码(选填)" class="regi_login_input" v-model="code"/>
            </li>
            <li class="regi_form_input noMargin">
              <i class="iconfont icon-address_icon"></i>
              <input type="text" tabindex="1" name="password" placeholder="请输入收货地址" class="regi_login_input" v-model="adders">
            </li>
          </ul>
        </div>
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="AddressShow=false">关闭</a>
          <a href="javascript:;" class="btn btn--m" @click="upAddress">添加</a>
        </div>
      </Modal>
      <NavFooter></NavFooter>
    </div>
...
<script>
  ...
  export default{
      ...
      methods:{
            ...
            if(res.data.status==0){
              ...
              this.addressList.forEach((item) => {
                ...
                if(item.isDefault){
                  ...
                  item.isDefault="1";
                }else if(!item.isDefault){item.isDefault="0"}
              })
              this.addressList = this.jsonSort(this.addressList,'isDefault','reverse')
              console.log('is addressList'+ res.data.result)
            }
          })
        },
        expand(){
        ...
        setDefault(addressId){
          axios.post('/users/setDefault',{addressId:addressId}).then(res=>{
            if(res.data.status ==0 ){
              console.log('is setDefault'+ res.data.result)
              this.getAddressList();
            }
          })
        },
        ...
        delAddress(){
            axios.post('/users/delAddress',{addressId:this.addressId}).then(res=>{
                if(res.data.status ==0){
                  console.log('is delAddress'+res.data.result)
                  this.isMdShow = false;
                  this.getAddressList();
                }
            })
        ...
        upAddress(){
          ...
          axios.post('/users/upAddress',{userName:this.addername,tel:this.phone,postCode:this.code,streetName:this.adders}).then(res=>{
            if(res.data.status == 0){
              alert('添加成功！')
              this.addername="";
              this.phone="";
        ...
        jsonSort(array,field,reverse){
          ...
          if(typeof array[0][field] === "string") {
            array.sort(function(x, y) {
              console.log(3)
              console.log(parseFloat(x[field]),parseFloat(y[field]))
      ...
      components:{
       NavHader,NavBread,NavFooter,Modal
      }
  }
</script>
